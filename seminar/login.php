<?php
	...
	if ( isset( $_SESSION['userSM'] ) ) {
		new Redirect( "seminara.php" );
	}
	...
	if ( $_SERVER["REQUEST_METHOD"] == "POST" ) {
		...
		$Conn = new Database;
		$isUser = $Conn->conn->prepare("Select `UserCd` from `infoseminar_users` where `UserCd` = '" . $u_code . "' and `Password` = '" . md5( $u_pass ) . "'");
		$isUser->execute();
...
<section class="form_login">
    <h2 class="h2Title"><span>セミナー　管理者画面　ログイン</span></h2>
