...
function saveSeminarC() {
    ...
    if ( !Number($('#CountPerson').val()) || Number($('#CountPerson').val()) < 0 ) {
        $('.error_inline0').html('席数を入力して下さい。');
...
function checkExistAllSeminarC() {
    jQuery.ajax({
        type: 'POST',
        dataType: 'json',
        url: _url_init,
