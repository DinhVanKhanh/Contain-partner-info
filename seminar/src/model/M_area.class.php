<?php
    class M_area extends Database {
        protected $table = 'infoseminar_areas';
        ...
        public function delete( int $id ) : string {
            try {
                $this->conn->exec( "DELETE FROM infoseminar_sumary WHERE AreaId = {$id}" );
                $this->conn->exec( "DELETE FROM infoseminar_todouhukens WHERE AreaId = {$id}" );
        ...
        public function exportCSV() : array {
            ...
            if ( !($result != false && count($result) > 0) ) {
                $rs['empty'] = "セミナー開催のデータがありません。";
                goto Result;
            }

            $outPut = $this->_prefix . 'EXPORT_AREAS_' . date('His') . '.csv';
            ...
            foreach ( $result as $data ) {
                // Code
                $code = htmlspecialchars( $data["AreaCode"] );
