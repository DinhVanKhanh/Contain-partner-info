<?php
    ...
    switch ( $_POST['action'] ) {
        ...
        case 'loadById':
            if ( preg_match( '/^(\s|\s+|\D)$/', $_POST['areaId'] ) ) {
        ...
        case 'changeOrderRow':
            if ( preg_match( '/^(\s|\s+|\D)$/', $_POST['curId'] ) ) {
                echo json_encode( ['errMsg' => 'Error current id'] );
            }
            elseif ( preg_match( '/^(\s|\s+|\D)$/', $_POST['upId'] ) ) {
                echo json_encode( ['errMsg' => 'Error change id'] );
            }
            elseif ( preg_match( '/^(\s|\s+|\D)$/', $_POST['curIdx'] ) ) {
                echo json_encode( ['errMsg' => 'Error current idx'] );
            }
            elseif ( preg_match( '/^(\s|\s+|\D)$/', $_POST['upIdx'] ) ) {
